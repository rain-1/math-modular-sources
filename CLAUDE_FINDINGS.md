...
- **Round 2 (corrections and advances).** The level-one negative control was a wrong-character artefact: with ψ = χ_{D₀} the congruence holds for every Paşol–Zudilin form, and at level one it is a THEOREM (exact T_{p²} identity on the Borcherds–Zagier basis ⇒ n² | β_ψ(n)). Kernel identified: γ(m) = (polar coefficient) × genus-twisted Heegner trace at D₀m² of the Shimura–Maass derivative of the weight −2 form 1/(xF) (70–135 digits, all rows); K is not modular, not even holonomic. Clean form for s₇: the weight-5/2 input has c(d) ∈ Z with d | c(d) for all 690 admissible d ≤ 2500 (conjecture = case d = m²; G(d) = c(d)/d vanishes iff 7 | d). Level-28 proof: uniqueness (dim S₄(Γ₀(7))^{W=−1} = 0) and eigenvalue λ ≡ p mod p² in place; the gap is an integral basis of vector-valued M^!_{5/2}(ρ_L) with the tower relation. New: A_{p−1} ≡ κψ(p)p mod p²; Ξ mod 2 is Eisenstein.
- **Master conjecture (verified n ≤ 12000, sharp):** n² | β(n); equivalently Φ = D³[Σ_e ψ(e)e⁻²K(q^e)] with K = Σγ(d)q^d integral, i.e. Ξ is an inner weight-3 Eisenstein series whose Lambert kernel q/(1−q) is replaced by an integral non-multiplicative K, and L(Φ,s) = L(s−1,ψ)·Σγ(d)d^{3−s}. Implies (S) and supercongruences such as c'(p²) ≡ ψ(p)c'(p) mod p⁴. In Shimura–Borcherds terms: a(|D|m²) = m³γ(m).
