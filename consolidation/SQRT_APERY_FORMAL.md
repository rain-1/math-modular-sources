...
**Definition 4.1 (nome).** $q\in u\,\mathbb Q[[u]]$ is the unique series $q=u+O(u^2)$ with
$$\theta_uq=\frac{q}{F\sigma}=\frac{q}{f^2\sigma}, \tag{4.1}$$
i.e. $\lambda:=\theta_u\log q=1/(f^2\sigma)$. (Existence and uniqueness: $\frac1{F\sigma}-1\in u\mathbb Q[[u]]$ determines $\log(q/u)$ coefficientwise; exponentiate. In Lean, define $q$ by the coefficient recursion that (4.1) gives.) Let $u(q)$ (equivalently $t(q)=4u(q)$) denote the compositional inverse.

**Theorem 4.2 (integrality of the nome; EXTERNAL).** $t(q)\in q\,\mathbb Z[[q]]$, $q(t)\in t\,\mathbb Z[[t]]$, and $F(t(q))\in1+q\,\mathbb Z[[q]]$.
*Status:* classical for Apéry's operator. Proof 1 (modular, Beukers 1987): $t(q)=\big(\eta(q)\eta(q^6)/\eta(q^2)\eta(q^3)\big)^{12}$ and $F(t(q))=(\eta(q^2)\eta(q^3))^7/(\eta(q)\eta(q^6))^5$ are eta quotients, hence in $\mathbb Z[[q]]$, and (4.1) is Beukers' identity $\theta_qt=tF\sqrt{1-34t+t^2}$. Proof 2 (elementary $p$-adic): Dwork's lemma applied to $q/t$, using the Dwork-type congruences of the Apéry numbers (Beukers 1985; Krattenthaler–Rivoal 2010). A formalization may take Theorem 4.2 as an axiom with these references, or formalize Proof 2. (Equivalent elementary form, Remark: $F\sqrt{1-34t+t^2}\in\mathbb Z[[t]]$, numerically $1-12t-156t^2-2964t^3-\cdots$.)
