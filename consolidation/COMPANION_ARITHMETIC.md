...
$$c(p^im)=c(p^i)\,c(m)\ (p\nmid m),\qquad c(p^i)\equiv\psi(p)^i\pmod{p^r}\tag{a}$$
(true for $\Phi=\sum_{d\mid N}\lambda_dE_{\psi,\chi,r}(d\tau)$ with $c_E(m)=\sum_{e\mid m}\chi(e)\psi(m/e)e^r$ and $p\nmid N$: multiplicativity, and $c_E(p^i)=\sum_{a\le i}\chi(p)^a\psi(p)^{i-a}p^{ra}$), and suppose the row satisfies the Dwork–Lucas congruence
