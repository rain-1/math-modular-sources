...
- **(E5) No second singularity at all.** (Added 2026-08-24, from `EMN_VERIFICATION.md`.) The Eskandari–Murty–Nemoto moving period ($G=\Phi(1)$, $\Phi(z)=\iint_\Delta dxdy/(1-z(x^2+y^2))$, arXiv:2510.20648) lives on a host whose only finite singularity is the fold $z=1$ itself — it is the $\mu_4$ dilogarithm module of `CATALAN_MU4.md` quotiented by $q\mapsto1/q$ and expanded at $q=1$ instead of at the cusp. The host is parabolic, the width law does not apply, and it **clears the entry condition** ($+0.096$, the first non-vacuous CDT bound for Catalan). It is nevertheless self-defeating: with no outer singularity the hypothesis $G\in\mathbb Q$ has nothing to remove (deleting the unique fold would make the module entire), so the conditional function buys $+1$ against a bound that rises by $\approx2.2$; best margin $-4.05$. Denominators of the row: $\mathrm{den}(a_n)\mid 2(n+1)[1..2n+1]$ exactly; $\delta=0.192$. Also verified there: $\Phi_3(3/2)=L_3(2,\chi_{12})$ to 89 3-adic digits (the class of the conductor-12 hypergeometric row of `TWO_PRIME_HOLONOMY.md`, and NOT the conductor-6 $\zeta_3(2)$ class).

- **(E4) Proving P2′.** Negates nothing in §2 — it is the lattice wall. P2′ is equivalent to $G\notin\mathbb Q$, so it needs analytic input about $G$; its measured form (ratio $\sqrt2$, slope $0$) is at least a statement one can try to attack through the arithmetic of the specific $2$-dimensional lattices $\mathcal K_n$ (Gauss reduction is explicit in dimension 2).  **Done: `P2_STRUCTURE.md` (2026-08-23).**  The
  outcome is that $v_1(\mathcal K_n)$ is always a continued-fraction convergent of $h_{12}/h_{11}$
  and lies in the cone **iff** that convergent's index is even (351/351 instances, $n\le120$), so
