> **Review note (Fable).** The transfer identity of §4 is linear in ξ and holds for every ξ (the A- and B-parts transfer separately), hence H3 ⇔ "R^F_{(5/4)ξ} has radius > 1" ⇔ the theorem. Theorem R is therefore a *reduction*, not a proof: Conjecture D for (C,F) at p=3 is equivalent to the 3-adic period identity for Θ_C under the Atkin–Lehner involution of X_0(12) (the 3-adic analogue of the fold lemma, which over ℂ supplies the w-invariance through the period polynomial). Everything else in this document stands. **Update (see `consolidation/PADIC_PERIOD.md`):** the involution is $\gamma=\binom{\ \,8\ -3}{12\ -4}=W_4$ on $X_0(12)$, and the archimedean analogue of H3 is *not* a vanishing statement: $\Theta_{\mathbf F}|_{-1}\gamma-\Theta_{\mathbf F}=\tfrac54\xi_\infty(6\tau-3)-i\tfrac{\pi^2}{6\sqrt3}(6\tau-1)\neq0$. The "*Reason.* Over $\mathbb C$ ..." paragraph of Proposition 3 is therefore withdrawn as a proof; H3 is 3-adic-only, and is verified pointwise to $3^{1600}$.
