...
print("v2(zeta_2(2)) = ", valuation(z2,2));
N=34; r=zudrow(N); u=r[1]; v=r[2];
print("m  v2(Pflat/Q - z2)   v2(P/Q - z2)  v2(Pflat/Q - Pflat_{m-1}/Q_{m-1})");
{ my(prev=0);
  for(m=1,30,
   my(b=bpq(m,1/2-m));
   my(c = (-1)^(m+1)*b[1]/8 - S(m)*u[m+1]);
   my(f = c/u[m+1]);
   print("  m=",m,"  vflat=",valuation(f-z2,2),"  vP=",valuation(v[m+1]/u[m+1]-z2,2), "  cauchy=",if(m>1,valuation(f-prev,2)));
   prev=f);
}
quit;
