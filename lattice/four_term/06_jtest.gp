...
NTERM  = 130;          /* series length */
DMAX   = 18;           /* deg J window: 6c+4e3+3e2-12 <= 18 for five singular points */
HMAX   = 18;           /* cusp width at the MUM point; h <= deg J <= 18 */
GLIM   = 2048;          /* gam = +- m^(+-h), m running over 7-smooth m <= GLIM */
...
nomegen(rn, rd, mm, j1, j2, aa, cc, dd, ff, cr, N, onec) =
{ my(be, ep, gg2, hh2, jj2, u, gv, y0, gser);
  be  = onec*aa*(rd - rn)/rd;            /* b = a(1-rho)            */
  ep  = -onec*2*rn*dd/rd;                /* e = -2 rho d            */
  gg2 = onec*cr*mm^2; hh2 = -onec*cr*mm*(j1+j2); jj2 = onec*cr*j1*j2;
  u = vector(N+2); gv = vector(N+2);
  u[1] = onec; gv[1] = 0*onec;
  for(n = 1, N+1,
    my(n1 = n-1,
       P0 = onec*aa*n1^2 + be*n1 + onec*cc,
       Q0 = onec*dd*n1^2 + ep*n1 + onec*ff,
       R0 = gg2*n1^2 + hh2*n1 + jj2,
       dP = 2*onec*aa*n1 + be,
       dQ = 2*onec*dd*n1 + ep,
       dR = 2*gg2*n1 + hh2);
    u[n+1] = (P0*u[n] - if(n >= 2, Q0*u[n-1], 0)
                      + if(n >= 3, R0*u[n-2], 0))/n^2;
    gv[n+1] = (dP*u[n] + P0*gv[n]
               - if(n >= 2, dQ*u[n-1] + Q0*gv[n-1], 0)
               + if(n >= 3, dR*u[n-2] + R0*gv[n-2], 0)
               - 2*n*u[n+1])/n^2;
  );
  y0   = sum(n = 0, N, u[n+1]*x^n) + O(x^(N+1));
  gser = sum(n = 0, N, gv[n+1]*x^n) + O(x^(N+1));
  x*exp(gser/y0);
}
...
certify(rn, rd, mm, j1, j2, aa, cc, dd, ff, cr, h, gam, dmax, N) =
{ my(qq, zz, base, mmax, zp, wv, es, nused, ker, vv, uu, gg, R, nextra, dg,
     wpol, wser);
  qq   = nomegen(rn, rd, mm, j1, j2, aa, cc, dd, ff, cr, N, 1);
  zz   = qq^h;
  base = x^h/zz;
  mmax = (N-1)\h;
...
jtest5(rn, rd, mm, j1, j2, aa, cc, dd, ff, cr, dmax = DMAX, hmax = HMAX, N = NTERM, p = KPRIME) =
{ my(onec, qq, glist, nq1);
  onec = Mod(1, p);
  qq = nomegen(rn, rd, mm, j1, j2, aa, cc, dd, ff, cr, N, onec);
  nq1 = dmax + 3;
  for(h = 1, hmax,
    ...
    for(gi = 1, #glist,
      ...
      cert = certify(rn, rd, mm, j1, j2, aa, cc, dd, ff, cr, h, gq, dmax, N);
