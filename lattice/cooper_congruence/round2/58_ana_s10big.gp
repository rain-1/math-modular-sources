\\ 58_ana_s10.gp -- divisibility analysis of c(d) for s10.  Raw data 57_cd_s10_big.txt holds
\\ A(d) = T(d)/i.  Test both normalisations c = -A (lambda=i) and c = -2A (lambda=2i).
read("50_lib.gp");
V = read("57_cd_s10_big.txt");
bet = read("20_beta_s10.txt");
print("s10: ", #V, " admissible d");
{ my(B=select(x->type(x[4])!="t_INT", V)); print("  non-integer/NOREP/DEGEN: ",#B," of them; all with 25|d ? ", vecmin(vector(#B,i,B[i][1]%25==0))==1); print("   they are: ",vector(#B,i,B[i][1])); }
