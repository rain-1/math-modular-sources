...
for(k=1,3,
  ...
  write(concat(concat("50_gamma_",NAM[k]),".txt"), g);
  write(concat(concat("50_beta_",NAM[k]),".txt"), b);
  write(concat(concat("50_cp_",NAM[k]),".txt"), cp);
);
}
print();
...
print();
print("== refined supercongruence 1b: exact modulus reached, p^k <= ",M,", k>=1 ==");
{
for(k=1,3,
  print("row ",NAM[k],":");
  forprime(p=2,53,
