...
doh(h) = my(N,C,B,dv,r,tag,us,Fs,xs,a,fr,mf,V,g,cv,Th,b,ker,P,lam1,lam2,fr2,ord,Q,av,bv,xi,wn,Kv,Km,Kp,Kpv,kappa,d0); N=h[1]; C=h[2]; B=h[3]; dv=h[4]; r=h[5]; tag=h[6]; fr=foldregC(N); if(fr[1]==0, return(0)); mf=fr[1]; V=fr[2]; if(matsize(V)[2]==0, return(0)); lam1=B+2*sqrt(C*1.0); lam2=B-2*sqrt(C*1.0); print(""); print("======== ", tag, "  lam1=", lam1, "  lam2=", lam2); us=useries(dv,r,NQ); Fs=Fseries(dv,r,NQ); xs=us/(1+B*us+C*us^2); a=peel2(Fs,xs,NA,NQ); ker=fitrecR(a,2,3); P=normPR(vector(3,j,sum(e=0,3,ker[(j-1)*4+e+1,1]*x^e))); d0 = polcoeff(P[1],0)/polcoeff(P[1],3); kappa = if(polcoeff(P[1],3)==0, 0, my(cc=-polcoeff(P[1],3), dd=-polcoeff(P[1],0)+0*cc); 0); av = iterhom(P, 2, [a[1],a[2]], NIT+6); Kpv = vector(22, i, av[NIT-21+i]*1.0*(NIT-22+i)^(3/2)/lam1^(NIT-22+i)); Kp = richfit(Kpv, NIT-22, 22); print("   K_+ measured        = ", Kp); print("   K_+ closed form     = ", sqrt(N*1.0)/(2*Pi^(3/2))*sqrt(lam1/(lam1-lam2))); for(j=1,matsize(V)[2], g=mflinear(mf, primvec(V[,j]~)~); cv=mfcoefs(g,NQ-1); if(cv[2]<0, cv=-cv; g=mflinear(mf,-primvec(V[,j]~)~)); Th=thetaser(cv,3,NQ); b=peel2(Fs*Th,xs,NA,NQ); fr2=findrec(b); ord=fr2[1]; Q=fr2[2]; if(ord==0, print("-- Phi[",j,"] no recurrence found"); next); bv = iterhom(Q, ord, vector(ord,i,b[i]), NIT+6); my(okk=1); for(i=1,NA-2, if(bv[i]!=b[i], okk=0; break)); print("-- Phi[",j,"] order-", ord, " recurrence reproduces B_n for n<=", NA-3, " : ", if(okk,"YES","NO")); xi = lfun(lfunmf(mf,g),3); Kv = vector(22, i, my(n=NIT-22+i); (xi*av[n+1]-bv[n+1])*n^(3/2)/lam2^n); Km = richfit(Kv, NIT-22, 22); print("   K_- (Richardson)    = ", Km); print("   K_+ K_- (lam1-lam2) = ", Kp*Km*(lam1-lam2)));
