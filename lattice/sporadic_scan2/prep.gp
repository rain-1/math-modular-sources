...
readsp(fn) = { my(L, h, dim, M);
  ...
  dim = h[6];
  if(dim == 0, return([h[2], h[4], 0, 0]));
  M = matrix(dim, #eval(L[2]), i, j, eval(L[i+1])[j]);
  [h[2], h[4], dim, M];
}

doprep(fn, out) = { my(R, N, w, dim, M, v, g, HU, U, c, F0, K, KB, T, red, nrm, ord, d);
  R = readsp(fn); N = R[1]; w = R[2]; dim = R[3];
  if(dim == 0, write(out, Str(N, " ", w, " 0 -1")); return(0));
  M = R[4];
  v = vectorv(dim, i, M[i,1]);          \\ constant terms
  g = content(v);
  if(g != 1, write(out, Str(N, " ", w, " ", dim, " -1")); return(0));
  HU = mathnf(v~, 1);                    \\ v~ is 1 x dim ; v~ * U = H
  U = HU[2];
  \\ find the column of U giving c with c.v = 1 : HNF puts it last
  c = U[, dim];
  if(c~ * v != 1, c = -c);
  if(c~ * v != 1, write(out, Str(N, " ", w, " ", dim, " -2")); return(0));
  K = matrix(dim, dim-1, i, j, U[i, j]);  \\ remaining columns span the kernel
  d = dim - 1;
  F0 = (c~ * M);
  if(d == 0,
    write(out, Str(N, " ", w, " ", dim, " 0"));
    write(out, Vec(F0));
    return(0));
