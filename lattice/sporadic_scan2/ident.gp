...
for(i = 1, #L,
  ...
  my(r1 = lindep([1, x]));
  my(msg = "");
  if(#r1 == 2 && r1[2] != 0 && vecmax(abs(r1)) < 100000 && abs(r1[1] + r1[2]*x) < 10.0^(-prec+8),
     msg = Str("RATIONAL ", -r1[1], "/", r1[2]),
     msg = tryrel(x, B, prec);
     if(msg == "",
        my(C = iferr(cuspbasis(M, w, prec+15), E, []));
        if(#C > 0, msg = tryrel(x, concat([[1,"1"]], C), prec));
        if(msg == "" && prec > 60, msg = trypair(x, B, prec));
        if(msg == "" && prec > 60 && #C > 0,
           msg = trypair(x, concat(concat([[1,"1"]], C), [B[2],B[3],B[4],B[5]]), prec));
     );
  );
  write(out, Str(idx, " | ", if(msg == "", "UNIDENTIFIED", msg)));
);
}
quit
