...
PX = 1670;  \\ order in x
PY = 830;   \\ order in y  (needs 2*(PY-1) <= PX-1)
...
{
report(nm, cv, pp) =
  my(nn, vv = vector(N1+2, ii, "oo"), t, lo, hi, cnt=0, sx=0,sy=0,sxx=0,sxy=0, sl, wide, v0, v1, lx, ly=0, lxx=0, lxy=0, lsx=0, al);
  ...
  for(nn = N0, min(N1, #cv-1),
    ...
    if(type(t) == "t_INT",
      ...
      lx = log(nn*1.0); lsx += lx; ly += t; lxx += lx^2; lxy += lx*t;
      if(lo == "" || t/nn < lo, lo = t/nn);
      if(hi == "" || t/nn > hi, hi = t/nn);
    );
  );
  sl = if(cnt >= 2, (cnt*sxy - sx*sy)/(cnt*sxx - sx^2)*1.0, "n/a");
  al = if(cnt >= 2, (cnt*lxy - lsx*ly)/(cnt*lxx - lsx^2), "n/a");
  v0 = vv[N0+1]; v1 = if(N1 < #cv, vv[N1+1], "oo");
  wide = if(type(v0)=="t_INT" && type(v1)=="t_INT", (v1-v0)/(N1-N0)*1.*1.0, "n/a");
    print("  p=", pp, "  v(c",N0,")=", v0, "  v(c",N1,")=", v1,
        ...
        "  nonzero=", cnt, "/", N1-N0+1);
  print("        log-fit  v ~ alpha*log(n)+beta :  alpha = ", if(type(al)=="t_REAL", strprintf("%+.4f", al), al), "   (= ", if(type(al)=="t_REAL", strprintf("%+.4f", al*log(pp*1.0)), "-"), " * log_", pp, "(n))");
  print("        v_",pp," at n=100,200,...,800: ", concat(vector(8, ii, Str(vv[100*ii+1], " "))));
}
