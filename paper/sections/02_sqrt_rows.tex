\section{Square roots of the sporadic rows, and an irrationality theorem}\label{sec:sqrt}
...
\begin{theorem}[irrationality of the square-root period]\label{thm:sqrt-irr}
...
$\Psi$ has unbounded denominators, so it is a non-congruence form and $\xi$ is not expected to be a classical $L$-value; an extensive search (95 structured constants including CM periods, 260 digits; \texttt{algdep} to degree 10) finds none. Theorem~\ref{thm:sqrt-irr} is, as far as we know, the first irrationality result for a critical value of a non-congruence modular form. Its mechanism is worth stating plainly: Ap\'ery's sequence is the symmetric square of a non-congruence sequence, and the square root has the \emph{same} archimedean geometry ($\lambda_1\lambda_2=16$, fold at the Fricke point) with one integration fewer, $k=2$; the score drops from $+0.525$ to $+0.139$ only because $\lambda_2$ is the square root of Ap\'ery's.
