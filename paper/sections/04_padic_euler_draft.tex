...
\begin{remark}[(a) is a condition on the source, $\sigma_p>0$ on the host]
\label{rem:zeta5caution}
\VerifiedR{exact; \texttt{lattice/theoremF\_hyp/zeta5\_check*.gp}}
Hypothesis \textup{(a)} is necessary but not sufficient, and it can even be
incompatible with \textup{(b)}.  On the level-$12$ $\zeta(5)$ host the purified
weight-six span is two-dimensional, with a Fricke-even source
...
$\tfrac18(7\Phi_{\rm even}+\Phi_{\rm odd})$, with oldform vector
$(1,-113,567,112,-1863,1296)$ on $d=(1,2,3,4,6,12)$ and
$L(\Phi,5)=\tfrac{31}{192}\zeta(5)$.  Since $t|W_{12}=t$ there, the failure of
$\Phi$ to be a $W_{12}$-eigenform is exactly the failure of
Theorem~\ref{thm:descentauto}: $H_{\xi^*}$ does not descend, \textup{(b)}
fails.  Independently \textup{(d)} fails: that host is $\mathrm{Sym}^4$ of
$\lambda^2-14\lambda+1$, so $c=\prod\lambda_i=1$ and $\sigma_2=0$ ---
equivalently its fold $t=7-4\sqrt3$ is a $2$-adic \emph{unit}, so the nearest
singularity sits on $|t|_2=1$ and $\rho=1$.  Accordingly $b_n/a_n$ does not
converge $2$-adically (increments pinned between $-1$ and $-3$).  By contrast
the level-$16$ host has $\Phi=(1,-85,1428,-5440,4096)$ a $W_{16}$-eigenform
with eigenvalue $+1$ and nearest singular $t$ of valuation $-1$, so $\rho=2$,
$\sigma_2=\log_2\rho=1$, and $\xi_2=\tfrac7{32}\zeta_2(5)$ to $372$ digits.
This suggests the general shape of the slope: $\sigma_p=\log_p\rho$ with $\rho$
the $p$-adic radius of the largest singularity-free disc around $t=0$
(for \eqref{eq:R2}--\eqref{eq:R3} the singular $t$ are the roots of $P_r$, of
product $1/c$, giving $\sigma_p=v_p(c)$).  Summarising:
\[
\begin{aligned}
\textup{(a)}+\textup{(b)}+\textup{(c)}+\textup{(d)}\ &\Rightarrow\
\lim_n b_n/a_n=-Q(w+1)\kappa_p,\\
\textup{(a)}+\textup{(b)}+\textup{(c)}\ &\Rightarrow\
