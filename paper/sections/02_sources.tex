...
$d\neq0$ ($s_7,s_{10},s_{18}$), which are of \emph{operator} order two despite
the cubic-looking recurrence \cite{Zagier2009,AlmkvistZudilin2006,Cooper2012}.
...
Theorem~\ref{thm:A} asserts nothing about $s_7,s_{10},s_{18}$.  Their canonical
nomes are integral ($t\in q\Z[[q]]$, no rescaling), but their $(t,F)$ are not
eta quotients in that normalization, and no weight-$3$ or weight-$4$
Eisenstein fit exists at levels $\{7,10,18\}$ and their divisors with the
natural characters \ExcludedR{stated bases}.  The obstruction is the
coordinate, not integrality; it would be wrong to say Cooper's three are not
Eisenstein.  Finding the Atkin--Lehner normalization or hauptmodul in which
they are eta-type is \Open{} \cite[Prob.~$\Phi$-2]{ProjSources}.
\end{remark}
