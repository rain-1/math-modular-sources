...
% Companion to sections/02_sources.tex (Zagier's six), 02_sqrt_rows.tex and
% 02_root_rows_draft.tex (the root rows).  Data: consolidation/HERFURTNER_CLASSIFICATION.md,
% scripts lattice/herfurtner/.
...
The complete list of primitive integral rows in those ranges is
Table~\ref{tab:herfrows}. Every one of them is already in the project's census;
the only ones whose $\ker L$ is an elliptic-surface local system are Zagier's six
and the two root rows of Theorem~\ref{thm:whichroots}. In particular \emph{no new
Apéry-like second-order row of Herfurtner type exists in this range}.
\end{enumerate}
\end{namedthm}
